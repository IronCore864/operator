...
class BindingMapping(Mapping[str, "Binding"]):
    ...
    def get(self, binding_key: Union[str, "Relation"]) -> "Binding":
        ...
        if isinstance(binding_key, Relation):
            ...
            raise ModelError(
                "binding key must be str or relation instance, not {}" "".format(
                    type(binding_key).__name__
                )
            )
...
def _format_action_result_dict(
    ...
    for key, value in input.items():
        ...
        if isinstance(value, MutableMapping):
            ...
            raise ValueError(
                "duplicate key detected in dictionary passed to 'action-set': {!r}".format(key)
            )
        else:
