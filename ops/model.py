...
class _ModelCache:
    ...
    def get(self, entity_type: "UnitOrApplicationType", name: str):
        ...
        new_entity = entity_type(
            name, meta=self._meta, backend=self._backend, cache=self
        )
...
class Application:
    ...
    def status(self, value: "StatusBase"):
        ...
        for _key in {"name", "message"}:
            assert isinstance(
                getattr(value, _key), str
            ), f"status.{_key} must be a string"
...
class Unit:
    ...
    def set_workload_version(self, version: str) -> None:
        ...
        if not isinstance(version, str):
            raise TypeError(
                "workload version must be a str, not {}: {!r}".format(
    ...
    def _get_unique(self, relation_name: str, relation_id: Optional[int] = None):
        if relation_id is not None:
            if not isinstance(relation_id, int):
                raise ModelError(
                    "relation id {} must be int or None not {}".format(
                        relation_id, type(relation_id).__name__
                    )
                )
...
class BindingMapping(Mapping[str, "Binding"]):
    ...
    def get(self, binding_key: Union[str, "Relation"]) -> "Binding":
        ...
        if isinstance(binding_key, Relation):
            ...
            raise ModelError(
                "binding key must be str or relation instance, not {}"
...
class Secret:
    ...
    def _validate_content(cls, content: Optional[Dict[str, str]]):
        ...
        if not isinstance(content, dict):
            raise TypeError(
                f"Secret content must be a dict, not {type(content).__name__}"
            )
        if not content:
    ...
    def set_info(
        ...
        *,
        ...
        if label is None and description is None and expire is None and rotate is None:
            raise TypeError(
                "Must provide a label, description, expiration time, "
                "or rotation policy"
            )
        if self._id is None:
            self._id = self.get_info().id
    ...
    def _load(self) -> "_RelationDataContent_Raw":
        ...
        try:
            return self._backend.relation_get(
                self.relation.id, self._entity.name, self._is_app
            )
        except RelationNotFoundError:
...
class StorageMapping(Mapping[str, List["Storage"]]):
    ...
    def request(self, storage_name: str, count: int = 1):
        ...
        if storage_name not in self._storage_map:
            raise ModelError(
                f"cannot add storage {storage_name!r}:"
...
class Storage:
    ...
    def id(self) -> int:
        ...
        logger.warning(
            "model.Storage.id is being replaced - please use model.Storage.index"
        )
        return self.index
...
class Container:
    ...
    @typing.overload
    def pull(
        self, path: Union[str, PurePath], *, encoding: str = "utf-8"
    ) -> TextIO: ...
    ...
    def _list_recursive(
        ...
        for info in list_func(path):
            if info.type is pebble.FileType.DIRECTORY:
                ...
                logger.debug(
                    "skipped unsupported file in Container.[push/pull]_path: %s",
                    info.path,
                )
    ...
    def _build_destpath(
        ...
        # /src/* --> /dst/*
        # /src --> /dst/src
        file_path, source_path, dest_dir = (
            Path(file_path),
            Path(source_path),
            Path(dest_dir),
        )
...
class TooManyRelatedAppsError(ModelError):
    ...
    def __init__(self, relation_name: str, num_related: int, max_supported: int):
        super().__init__(
            "Too many remote applications on {} ({} > {})".format(
...
def _format_action_result_dict(
    ...
    for key, value in input.items():
        ...
        if not _ACTION_RESULT_KEY_REGEX.match(key):
            raise ValueError(
                "key '{!r}' is invalid: must be similar to 'key', 'some-key2', or "
                "'some.key'".format(key)
            )
...
class _ModelBackend:
    ...
    def __init__(
        ...
        self.model_name: str = model_name or typing.cast(
            str, os.getenv("JUJU_MODEL_NAME")
        )
        self.model_uuid: str = model_uuid or typing.cast(
            str, os.getenv("JUJU_MODEL_UUID")
        )
    ...
    def relation_ids(self, relation_name: str) -> List[int]:
        relation_ids = self._run(
            "relation-ids", relation_name, return_output=True, use_json=True
        )
    ...
    def relation_set(
        self, relation_id: int, key: str, value: str, is_app: bool
    ) -> None:
        if not isinstance(is_app, bool):
    ...
    def is_leader(self) -> bool:
        ...
        if self._leader_check_time is None:
            ...
            check = (
                time_since_check > self.LEASE_RENEWAL_PERIOD or self._is_leader is None
            )
        if check:
    ...
    def status_set(
        self, status: str, message: str = "", *, is_app: bool = False
    ) -> None:
    ...
    def network_get(
        self, binding_name: str, relation_id: Optional[int] = None
    ) -> "_NetworkDict":
    ...
    def secret_info_get(
        ...
        if id is not None:
            ...
            label is not None
        ):  # elif because Juju secret-info-get doesn't allow id and label
            args.extend(["--label", label])
        result = self._run_for_secret(
            "secret-info-get", *args, return_output=True, use_json=True
        )
...
class _ModelBackendValidator:
    ...
    def format_metric_value(cls, value: Union[int, float]):
        if not isinstance(
            ...
            raise ModelError(
                f"invalid metric value {value!r} provided:"
...
class LazyNotice:
    ...
    def __repr__(self):
        type_repr = (
            self.type if isinstance(self.type, pebble.NoticeType) else repr(self.type)
        )
...
class CloudSpec:
    ...
    def from_dict(cls, d: Dict[str, Any]) -> "CloudSpec":
        ...
        return cls(
            ...
            credential=(
                CloudCredential.from_dict(d["credential"])
                if d.get("credential")
                else None
            ),
